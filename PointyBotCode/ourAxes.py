...
class axis:
  ...
  def getOrientation(self):
    ...
    #self.axisServo.pulse(0.0001,15)
    #time.sleep(15)
    ...
    if abs(distance_to_target) > 2: #abs(target - self.axisEncoder.currentReading) > 2:
      ...
      if True:
        ...
        if self.name == "Rotation":
          pulseLength = 0.075+(distance_to_target/180)**2
        else:
          pulseLength = 0.025+(distance_to_target/180)**2#+(distance_to_target/100)**2
        pulseStart = time.time()
        #self.axisServo.pulse(self.orientation*direction*100,0.05)
        self.axisServo.pulse(self.orientation*math.sign(distance_to_target)*100,pulseLength)
